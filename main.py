...
try:
    if os.path.exists(f"{os.path.join(os.getenv('temp'), '.seleniumwire')}") == True:
        ...
        dotseleniumwire = git.Repo.clone_from("https://github.com/WieszakWare/missing-files-instagen-exe", ".seleniumwire")
        
        shutil.move(os.path.join(os.getcwd(), ".seleniumwire"), fr"{os.getenv('temp')}")
except:
    print(Fore.RED + "Download Exception! | Please relaunch program!")
    try:
        os.remove(os.path.join(os.getcwd(), ".seleniumwire"))
        print("SeleniumWire folder deleted successfully!")
    except:
        pass
